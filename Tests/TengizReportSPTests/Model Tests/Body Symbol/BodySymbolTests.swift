...
final class BodySymbolPatternTests: XCTestCase {
    func test_bodyHeader() {
        ...
        // no match
        XCTAssertNil("Основные расходы:        20%    8.95%".bodyHeader(), "Mind TAB and \t")
        XCTAssertNil("Основные расходы:    -----------------------------    25%".bodyHeader(), "Mind TAB and \t")
    ...
    func test_bodyFooter() {
        XCTAssertEqual(Patterns.bodyFooter, #"(?=ИТОГ:)(?<title>^.*?)(?:\t\s*)(?<value>\d{1,3}(?:\.\d{3})*(?:р \d\d? ?к)?)$"#)
    ...
    func test_rubKop() {
        XCTAssertEqual(Patterns.rubKop, #"\d{1,3}(?:\.\d{3})*(?:р \d\d? ?к)?"#)
...
final class BodySymbolTests: XCTestCase {
    ...
    func test_BodySymbol_init_footer() {
        ...
        XCTAssertEqual(BodySymbol(stringLiteral: input),
                       .footer(title: "ИТОГ:", value: 65_167))
    ...
    func test_BodySymbol_init_items_special_cases() {
        XCTAssertEqual(BodySymbol("1. Приход товара по накладным\t922.936р37к (оплаты фактические: 313.570р 26к-переводы; 87.091р 20к-корпоративная карта; 97.712-наличные из кассы; Итого 498.373р46к)"),
                       .item(title: "1. Приход товара по накладным", value: 498_373.46, comment: "922.936р37к (оплаты фактические: 313.570р 26к-переводы; 87.091р 20к-корпоративная карта; 97.712-наличные из кассы; Итого 498.373р46к)"),
